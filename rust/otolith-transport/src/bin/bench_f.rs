//! Contender F bench main: Rust iceoryx2 backend (ADR-0006, M2).
//!
...
use otolith_transport::iox2::{Iox2Transport, SERVICE_F};
use otolith_transport::{BenchMsg, Transport};
...
fn run_pub(a: &Args) -> ExitCode {
    let mut ring = match Iox2Transport::publisher(SERVICE_F) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("iox2 publisher: {e}");
...
fn run_sub(a: &Args) -> ExitCode {
    let mut ring = match Iox2Transport::subscriber(SERVICE_F) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("iox2 subscriber: {e}");
            return ExitCode::FAILURE;
        }
    };
...
fn main() -> ExitCode {
    let Some(a) = parse_args() else {
        eprintln!("usage: bench_f --role pub|sub --rate HZ --n N --out PATH [--ready PATH]");
