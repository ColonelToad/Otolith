...
Foxglove: `ros2/otolith_description` (URDF `go2.urdf` + binary STL `meshes/*.stl` converted from Menagerie OBJs — Foxglove ignores `.mtl` and lists OBJ as its weakest format; latched `/robot_description`) + `robot_state_publisher` (`joint_states` → `tf` for 12 joints) + `ros2/otolith_fusion` (`fusion_node.cpp:14`: `world→base` (est) and `world→base_gt` `tf`, `gt_path`/`est_path` `Path` at 10 Hz, covariance ellipsoid `MarkerArray` gated behind `publish_covariance:=false` by default — the 4σ blob is bigger than the robot and hides the mesh, and a panel color override paints it opaque green) → `foxglove_bridge` on `:8765` → Foxglove Studio (Windows) `foxglove/go2_demo.json` (3D URDF Source=Topic `/robot_description`, Scene mesh-up-axis Z-up + Studio restart, TF + Paths, markers hidden for video). Heaviness reduction: URDF meshes are sent once (latched `transient_local`), `tf`+`Path` are throttled (not 500 Hz); the 500 Hz `state_estimate` stays on `BEST_EFFORT`. Full mesh is shown without per-frame mesh bytes.
