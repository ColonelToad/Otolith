...
Foxglove: `ros2/otolith_description` (URDF `go2.urdf` + binary STL `meshes/*.stl` converted from Menagerie OBJs — Foxglove ignores `.mtl` and lists OBJ as its weakest format; latched `/robot_description`) + `robot_state_publisher` (`joint_states` → `tf` for 12 joints) + `ros2/otolith_fusion` (`fusion_node.cpp:14`: `world→base` (est) and `world→base_gt` `tf`, `gt_path` `Path` @100 Hz / `est_path` throttled to ~50 Hz, covariance ellipsoid `MarkerArray` gated behind `publish_covariance:=false` by default — the 4σ blob is bigger than the robot and hides the mesh, and a panel color override paints it opaque green) → `foxglove_bridge` on `:8765` → Foxglove Studio (Windows) `foxglove/go2_demo.json` (3D URDF Source=Topic `/robot_description`, Scene mesh-up-axis Z-up + Studio restart, TF + Paths, markers hidden for video). Heaviness reduction: URDF meshes are sent once (latched `transient_local`), `tf` is throttled and `est_path` is throttled to ~50 Hz (not 500 Hz); `gt_path` publishes at the 100 Hz GT rate; the 500 Hz `state_estimate` stays on `BEST_EFFORT`. Full mesh is shown without per-frame mesh bytes.
...
## 10. v0.4 — RTL port: fixed-point predict pipeline, kernel GDS (M4)

v0.4 ports the predict step to synthesizable SystemVerilog in Q8.24 (states/Phi) + Q16.48 (covariance P) with 128-bit accumulators, round-half-away, saturate-and-count — formats from measured ranges, not assumption (P spans 11 decades; Q8.24 alone diverges to 3.87 m). Bit-parity vs the C++ model holds 2500/2500 steps (corr 0.999984, hybrid 0.1023 m vs float 0.1064 m). ECP5: Fmax 65.8 MHz, 117 µs/step (17× in budget, jitter none by construction).

The SKY130 headline is two numbers, not one. The full fixed-point predict core synthesizes to **297,819 cells / 3.40 mm²** — the honest cost of building it, and the reason the full-core GDS is a documented dead end (134% over the die; pre-PnR STA reporting OOMs on the all-violated netlist). The closed silicon loop runs on the dominant operator, the registered 32×64 multiply-narrow kernel: **GDS 21.9 MB, klayout XOR 0 diffs, route DRC 0, antenna 0, hold MET all corners, 0.119 mm² @19.3% util, 0.297 W, Fmax ~67 MHz TT / ~36 MHz SS** at 10 ns target — reported as achieved, not as closed. v0.4's answer: the fixed-point port is real silicon-shape arithmetic with measured PPA, and the full core has a price tag. Cholesky (update path) is the open stretch.
