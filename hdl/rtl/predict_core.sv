...
// deterministic ~6.8k cycles/step. Unrolling the cheap stuff +
...
// round-half-away, same saturation. 64-bit accumulations use s_add64
...
module predict_core (
  ...
  //   bg    1e-10  -> 28147
  //   ba    1e-8   -> 2814750
  localparam q48_t SIG2_G  = 64'd28147497671;
  localparam q48_t SIG2_A  = 64'd6333186975990;
  localparam q48_t SIG2_BG = 64'd28147;
  localparam q48_t SIG2_BA = 64'd2814750;

  localparam q24_t ONE_Q = 32'sd16777216; // 1.0, avoid magic below
  localparam q24_t TWO_Q = 32'sd33554432; // 2.0 exact

  // ---------- registers ----------
  q24_t gyro[3], accel[3], dtf, gf;       // inputs
  q24_t q[4], p[3], v[3], bg[3], ba[3];   // nominal state
  q48_t P[225];                            // covariance, Q16.48
  q24_t Phi[225];                          // rebuilt every step
  // Hoisted nominal temps (stable snapshot sources for dbg regs).
  q24_t R_[9], eq_[4], nq_[4];
  q48_t P1[225], P2[225];                  // engine outputs
  q48_t qd_g, qd_a, qd_bg, qd_ba;          // Qd consts for this dt
  ...
  q24_t dbg_R0, dbg_R4, dbg_R8, dbg_eq0, dbg_nq0, dbg_Phi0;
  q48_t dbg_qd_g;
  q24_t dbg_Phi9; // Phi[0][9] (F09 block) — P-rail bisect
  ...
  q24_t  n_q[4], n_p[3], n_v[3];
  q24_t  n_Phi[225];
  q48_t  n_qd_g, n_qd_a, n_qd_bg, n_qd_ba;
  logic [31:0] nom_sat;
  ...
  always_comb begin : nominal
    ...
    for (i = 0; i < 3; i++) begin
      r = s_sub24(gyro[i], bg[i]); sc += {31'b0, r.sat}; w[i] = r.v;
      r = s_sub24(accel[i], ba[i]); sc += {31'b0, r.sat}; av[i] = r.v;
    end
    // R = quat_to_mat(q): 9 entries, same formulas as model
    begin
      q24_t qw, qx, qy, qz, t, u;
      qw = q[0]; qx = q[1]; qy = q[2]; qz = q[3];
      // R_[0] = 1-2(qy^2+qz^2)
      r = s_mul24(qy, qy); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qz, qz); sc += {31'b0, r.sat};
      r = s_add24(t, r.v); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; t = r.v;
      r = s_sub24(ONE_Q, t); sc += {31'b0, r.sat}; R_[0] = r.v;
      // R_[1] = 2(qx*qy - qz*qw)
      r = s_mul24(qx, qy); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qz, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_sub24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[1] = r.v;
      // R_[2] = 2(qx*qz + qy*qw)
      r = s_mul24(qx, qz); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qy, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[2] = r.v;
      // R_[3] = 2(qx*qy + qz*qw)
      r = s_mul24(qx, qy); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qz, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[3] = r.v;
      // R_[4] = 1-2(qx^2+qz^2)
      r = s_mul24(qx, qx); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qz, qz); sc += {31'b0, r.sat};
      r = s_add24(t, r.v); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; t = r.v;
      r = s_sub24(ONE_Q, t); sc += {31'b0, r.sat}; R_[4] = r.v;
      // R_[5] = 2(qy*qz - qx*qw)
      r = s_mul24(qy, qz); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qx, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_sub24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[5] = r.v;
      // R_[6] = 2(qx*qz - qy*qw)
      r = s_mul24(qx, qz); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qy, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_sub24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[6] = r.v;
      // R_[7] = 2(qy*qz + qx*qw)
      r = s_mul24(qy, qz); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qx, qw); sc += {31'b0, r.sat}; u = r.v;
      r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; R_[7] = r.v;
      // R_[8] = 1-2(qx^2+qy^2)
      r = s_mul24(qx, qx); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(qy, qy); sc += {31'b0, r.sat};
      r = s_add24(t, r.v); sc += {31'b0, r.sat}; t = r.v;
      r = s_mul24(TWO_Q, t); sc += {31'b0, r.sat}; t = r.v;
      r = s_sub24(ONE_Q, t); sc += {31'b0, r.sat}; R_[8] = r.v;
    end
    nom_sat = sc;
    // --- exp: wdt, halve->ev, en2 (s_add64 chain from ONE^2), invsqrt, eq
    begin
      q24_t wdt[3], evloc[3], einv;
      logic signed [63:0] en2acc;

      for (i = 0; i < 3; i++) begin
        r = s_mul24(w[i], dtf); sc += {31'b0, r.sat}; wdt[i] = r.v;
        r = s_halve24(wdt[i]); evloc[i] = r.v; // halve: no sat possible
      end
      ...
      for (i = 0; i < 3; i++) begin
        r64 = s_add64(en2acc, 64'(evloc[i]) * 64'(evloc[i]));
        sc += {31'b0, r64.sat}; en2acc = r64.v;
      end
      r = s_narrow(en2acc); sc += {31'b0, r.sat};
      begin

        ri = invsqrt6(r.v); sc += {31'b0, ri.sat}; einv = ri.v;
      end
      r = s_mul24(ONE_Q, einv); sc += {31'b0, r.sat}; eq_[0] = r.v;
      for (i = 0; i < 3; i++) begin
        r = s_mul24(evloc[i], einv); sc += {31'b0, r.sat}; eq_[i+1] = r.v;
      end
      for (i = 0; i < 3; i++) ev[i] = evloc[i];
      // --- qprod nq (Hamilton, left-assoc, mirror of model) ---
      begin
        q24_t q0, q1, q2, q3, e0, e1, e2, e3, t, u;
        
        q0 = q[0]; q1 = q[1]; q2 = q[2]; q3 = q[3];
        e0 = eq_[0]; e1 = eq_[1]; e2 = eq_[2]; e3 = eq_[3];
        // nq[0] = ((q0*e0 - q1*e1) - q2*e2) - q3*e3
        r = s_mul24(q0, e0); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q1, e1); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q2, e2); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q3, e3); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; nq_[0] = r.v;
        // nq[1] = ((q0*e1 + q1*e0) + q2*e3) - q3*e2
        r = s_mul24(q0, e1); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q1, e0); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q2, e3); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q3, e2); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; nq_[1] = r.v;
        // nq[2] = ((q0*e2 + q2*e0) + q3*e1) - q1*e3
        r = s_mul24(q0, e2); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q2, e0); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q3, e1); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q1, e3); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; nq_[2] = r.v;
        // nq[3] = ((q0*e3 + q3*e0) + q1*e2) - q2*e1
        r = s_mul24(q0, e3); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q3, e0); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q1, e2); sc += {31'b0, r.sat}; u = r.v;
        r = s_add24(t, u); sc += {31'b0, r.sat}; t = r.v;
        r = s_mul24(q2, e1); sc += {31'b0, r.sat}; u = r.v;
        r = s_sub24(t, u); sc += {31'b0, r.sat}; nq_[3] = r.v;
        ...
        begin
          ...
          qn2 = 0;
          for (i = 0; i < 4; i++) begin
            r64 = s_add64(qn2, 64'(nq_[i]) * 64'(nq_[i]));
            sc += {31'b0, r64.sat}; qn2 = r64.v;
          end
          r = s_narrow(qn2); sc += {31'b0, r.sat};
          ri2 = invsqrt6(r.v); sc += {31'b0, ri2.sat};
          for (i = 0; i < 4; i++) begin
            r = s_mul24(nq_[i], ri2.v); sc += {31'b0, r.sat}; n_q[i] = r.v;
          end
        end
      end
      // --- v/p: Ra = R*av (s_add64 chains), dv/dp ---
      begin
        q24_t Ra[3], dv, dpv, gvv;
        gvv = 0;
        for (i = 0; i < 3; i++) begin
          ...
          sum = 0;
          for (j = 0; j < 3; j++) begin
            r64 = s_add64(sum, 64'(R_[i*3+j]) * 64'(av[j]));
            sc += {31'b0, r64.sat}; sum = r64.v;
          end
          r = s_narrow(sum); sc += {31'b0, r.sat}; Ra[i] = r.v;
        end
        // gvec = (0,0,-gf)
        for (i = 0; i < 3; i++) begin
          q24_t gi;
          if (i == 2) begin
            ...
            rn = s_neg24(gf); sc += {31'b0, rn.sat}; gi = rn.v;
          end else gi = 32'sd0;
          r = s_add24(Ra[i], gi); sc += {31'b0, r.sat};
          r = s_mul24(r.v, dtf); sc += {31'b0, r.sat}; dv = r.v;
          r = s_add24(v[i], dv); sc += {31'b0, r.sat};
          gvv = r.v; // reuse temp: updated v
          r = s_mul24(gvv, dtf); sc += {31'b0, r.sat}; dpv = r.v;
          r = s_add24(p[i], dpv); sc += {31'b0, r.sat};
          n_v[i] = gvv;
          n_p[i] = r.v;
        end
      end
      // --- F30 = -R*skew(av): 27 MACs via s_add64 chains ---
      begin
        q24_t Sa[9];
        // skew(av): [0,-az,ay; az,0,-ax; -ay,ax,0]
        Sa[0] = 0; Sa[4] = 0; Sa[8] = 0;
      begin
          rn = s_neg24(av[2]); sc += {31'b0, rn.sat}; Sa[1] = rn.v;
          Sa[2] = av[1];
          Sa[3] = av[2];
          rn = s_neg24(av[0]); sc += {31'b0, rn.sat}; Sa[5] = rn.v;
          rn = s_neg24(av[1]); sc += {31'b0, rn.sat}; Sa[6] = rn.v;
          Sa[7] = av[0];
        end
        for (i = 0; i < 3; i++) begin
          for (j = 0; j < 3; j++) begin
            ...
            fsum = 0;
            for (k = 0; k < 3; k++) begin
              r64 = s_add64(fsum, 64'(R_[i*3+k]) * 64'(Sa[k*3+j]));
              sc += {31'b0, r64.sat}; fsum = r64.v;
            end
            rn2 = s_narrow(fsum); sc += {31'b0, rn2.sat};
            rn2 = s_neg24(rn2.v); sc += {31'b0, rn2.sat};
            F30[(i*3+j)] = rn2.v;
          end
        end
      end
      ...
      begin
        ...
        F00[0] = 0;
      begin
          rn = s_neg24(w[2]); sc += {31'b0, rn.sat};
          rn = s_neg24(rn.v); sc += {31'b0, rn.sat}; F00[1] = rn.v;
          rn = s_neg24(w[1]); sc += {31'b0, rn.sat}; F00[2] = rn.v;
          rn = s_neg24(w[2]); sc += {31'b0, rn.sat}; F00[3] = rn.v;
          rn = s_neg24(w[0]); sc += {31'b0, rn.sat};
          rn = s_neg24(rn.v); sc += {31'b0, rn.sat}; F00[5] = rn.v;
          rn = s_neg24(w[1]); sc += {31'b0, rn.sat};
          rn = s_neg24(rn.v); sc += {31'b0, rn.sat}; F00[6] = rn.v;
          rn = s_neg24(w[0]); sc += {31'b0, rn.sat}; F00[7] = rn.v;
          F00[4] = 0; F00[8] = 0;
        end
        // F312 = -R
        for (i = 0; i < 9; i++) begin

          rn = s_neg24(R_[i]); sc += {31'b0, rn.sat}; F312[i] = rn.v;
        end
        for (i = 0; i < 15; i++) begin
          for (j = 0; j < 15; j++) begin
            ...
            if (fz) begin
              ...
              r = s_mul24(f, dtf); sc += {31'b0, r.sat};
              if (i == j) begin

                r2 = s_add24(ONE_Q, r.v); sc += {31'b0, r2.sat};
                n_Phi[i*15+j] = r2.v;
      ...
      begin
        ...
        rq = s_narrow48(acc128_t'(SIG2_G) * acc128_t'(dtf)); sc += {31'b0, rq.sat}; n_qd_g = rq.v;
        rq = s_narrow48(acc128_t'(SIG2_A) * acc128_t'(dtf)); sc += {31'b0, rq.sat}; n_qd_a = rq.v;
        rq = s_narrow48(acc128_t'(SIG2_BG) * acc128_t'(dtf)); sc += {31'b0, rq.sat}; n_qd_bg = rq.v;
        rq = s_narrow48(acc128_t'(SIG2_BA) * acc128_t'(dtf)); sc += {31'b0, rq.sat}; n_qd_ba = rq.v;
      end
      nom_sat = sc;
    end
  end

  // ---------- QDSYM combinational datapath (from P2 regs + qd consts) ----------
  // Mirror of model Qd+symmetrize: v = P2 + diag(Qd), sym = (v+v')/2 rounded.
  q48_t  n_P[225];
  logic [31:0] qd_sat;
  always_comb begin : qdsym

    logic [31:0] sc;
    sat48_t r1;
    q48_t vv, tt;
    acc128_t halves, h, sym;
    sc = 0;
    // Default-assign conditionally-written temps (else latches).
    r1 = '0;
    vv = 0; tt = 0; halves = 0; h = 0; sym = 0;
    for (int rr = 0; rr < 15; rr++) begin
      for (int cc = 0; cc < 15; cc++) begin
        vv = P2[rr*15+cc];
        if (rr == cc) begin
          if (rr < 3) begin r1 = s_add48(vv, qd_g); sc += {31'b0, r1.sat}; vv = r1.v; end
          else if (rr < 6) begin r1 = s_add48(vv, qd_a); sc += {31'b0, r1.sat}; vv = r1.v; end
          else if (rr < 9) begin end // dp: none
          else if (rr < 12) begin r1 = s_add48(vv, qd_bg); sc += {31'b0, r1.sat}; vv = r1.v; end
          else begin r1 = s_add48(vv, qd_ba); sc += {31'b0, r1.sat}; vv = r1.v; end
        end
        tt = P2[cc*15+rr];
        begin
          halves = acc128_t'(vv) + acc128_t'(tt);
          h = (halves >= 0) ? acc128_t'(1) : -acc128_t'(1);
          // ARITHMETIC shift: logical >> on negatives inserts zeros and
          // rails every negative entry (a real bug caught by parity).
          sym = (halves + h) >>> 1;
          if (sym > acc128_t'(Q48_MAX)) begin sc += 1; n_P[rr*15+cc] = Q48_MAX; end
          else if (sym < acc128_t'(Q48_MIN)) begin sc += 1; n_P[rr*15+cc] = Q48_MIN; end
          else n_P[rr*15+cc] = sym[63:0];
        end
      end
    end
    qd_sat = sc;
    ...
    if (rst) begin
      ...
      case (state)
        ...
        ST_P1, ST_P2: begin
          ...
          acc128_t prod;
          sat48_t nr;
          prod = acc128_t'(eng_a) * acc128_t'(eng_b);
          if (mm_k == 14) begin
            nr = s_narrow48(mm_acc + prod);
        ...
        ST_QDSYM: begin
          for (int n = 0; n < 225; n++) P[n] <= n_P[n];
          sat_cnt <= sat_cnt + qd_sat;
          state <= ST_DONE;
        end
        ST_DONE: begin
          done_r <= 1'b1;
